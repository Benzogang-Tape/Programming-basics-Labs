...
void init_dymanic_matrix(double** matrix, const unsigned N, const unsigned M) {
	for (size_t i{}; i < N; i++) {
		for (size_t j{}; j < M; j++) {
			if (i < j) {
				if ((j - i) > 1)
					...
					matrix[i][j] = (pow(constants::x, i + 1) / pow(fact(j + 1), i + 1));
			}
			else if (i > j) {
				if (j)
					matrix[i][j] = matrix[i][j - 1] / pow(i + 1, j + 1);
				else
					matrix[i][j] = pow(-constants::x, i + 1) / pow(fact(j + 1), i + 1);
			}
			else {
...
void init_static_matrix(double matrix[][constants::static_Matrix_Size], const unsigned N) {
	for (unsigned i{}; i < N; ++i) {
...
void delete_matrix(double**& matrix, const unsigned N, const unsigned M) {
	for (unsigned i{}; i < N; ++i) {
		delete [] matrix[i];
	}
	delete[] matrix;
