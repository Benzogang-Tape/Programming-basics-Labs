...
methods met = { simple_iterations, half_division };
functions fun = { transformed_function, equation, derivative };
...
void solution(double k, double eps, double approximation) {
	...
	unsigned iter = 1;
	for (const auto& meth : met) {
		std::cout << std::format("{:<16}{:<16.7f}{}", iter, meth(k, fun[iter - 1], eps, approximation), (*iterations_count)) << std::endl;
		iter++;
	}
	std::cout << std::format("{:<16}{:<16.7f}{}", iter, newton_method(k, fun[1], fun[2], eps, approximation), (*iterations_count)) << std::endl << std::endl;
}
